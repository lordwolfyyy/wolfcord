...
def get_signature_parameters(
    ...
    params = {}
    cache: Dict[str, Any] = {}
    eval_annotation = wolfcord.utils.evaluate_annotation
    ...
    async def _parse_arguments(self, ctx: Context) -> None:
        ...
        if self.cog is not None:
            ...
            try:
                ...
                raise wolfcord.ClientException(
        ...
        try:
            ...
            if cog is not None:
                ...
                if local_check is not None:
                    ret = await wolfcord.utils.maybe_coroutine(local_check, ctx)
                    if not ret:
                        return False
            ...
            return await wolfcord.utils.async_all(predicate(ctx) for predicate in predicates)  # type: ignore
        finally:
...
def has_role(item: Union[int, str]) -> Callable[[T], T]:
    ...
    def predicate(ctx: Context) -> bool:
        ...
        if isinstance(item, int):
            role = wolfcord.utils.get(ctx.author.roles, id=item)  # type: ignore
        else:
            role = wolfcord.utils.get(ctx.author.roles, name=item)  # type: ignore
        if role is None:
...
def has_any_role(*items: Union[int, str]) -> Callable[[T], T]:
    ...
    def predicate(ctx) -> bool:
        ...
        # ctx.guild is None doesn't narrow ctx.author to Member
        getter = functools.partial(wolfcord.utils.get, ctx.author.roles)
        if any(
...
def bot_has_role(item: int) -> Callable[[T], T]:
    ...
    def predicate(ctx) -> bool:
        ...
        me = ctx.me
        if isinstance(item, int):
            role = wolfcord.utils.get(me.roles, id=item)
        else:
            role = wolfcord.utils.get(me.roles, name=item)
        if role is None:
...
def bot_has_any_role(*items: int) -> Callable[[T], T]:
    ...
    def predicate(ctx) -> bool:
        ...
        me = ctx.me
        getter = functools.partial(wolfcord.utils.get, me.roles)
        if any(
            getter(id=item) is not None if isinstance(item, int) else getter(name=item) is not None
            for item in items
        ):
...
def has_permissions(**perms: bool) -> Callable[[T], T]:
    ...
    invalid = set(perms) - set(wolfcord.Permissions.VALID_FLAGS)
    if invalid:
...
def is_nsfw() -> Callable[[T], T]:
    ...
    def pred(ctx: Context) -> bool:
        ...
        if ctx.guild is None or (
            isinstance(ch, (wolfcord.TextChannel, wolfcord.Thread)) and ch.is_nsfw()
        ):
            return True
...
def dynamic_cooldown(
    ...
    accepts a single parameter of type :class:`.wolfcord.Message` and must
    ...
    Parameters
    ----------
    cooldown: Callable[[:class:`.wolfcord.Message`], Optional[:class:`.Cooldown`]]
