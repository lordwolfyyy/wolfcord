...
if TYPE_CHECKING:
    ...
    from wolfcord.activity import BaseActivity
    from wolfcord.enums import Status
    from wolfcord.flags import MemberCacheFlags
    from wolfcord.mentions import AllowedMentions
    from wolfcord.message import Message
...
MISSING: Any = wolfcord.utils.MISSING
...
class MissingMessageContentIntentWarning(UserWarning):
    """Warning category raised when instantiating a :class:`~wolfcord.ext.commands.Bot` with a
    :attr:`~wolfcord.ext.commands.Bot.command_prefix` but without the :attr:`~wolfcord.Intents.message_content`
    intent enabled.

    This warning is not raised when the :attr:`~wolfcord.ext.commands.Bot.command_prefix`
    is set to an empty iterable or :func:`when_mentioned <wolfcord.ext.commands.when_mentioned>`.
...
class BotBase(GroupMixin):
        ...
        *,
        ...
        if (
            ...
            warnings.warn(
                ...
                "See https://docs.wolfcord.dev/en/stable/intents.html#what-happened-to-my-prefix-commands "
    ...
    @wolfcord.utils.copy_doc(wolfcord.Client.close)
    ...
    async def can_run(self, ctx: Context, *, call_once: bool = False) -> bool:
        ...
        return await wolfcord.utils.async_all(f(ctx) for f in data)  # type: ignore

    async def is_owner(self, user: wolfcord.User) -> bool:
        """|coro|
        Checks if a :class:`~wolfcord.User` or :class:`~wolfcord.Member` is the owner of
        this bot.
    ...
    def add_cog(self, cog: Cog, *, override: bool = False) -> None:
        ...
        if existing is not None:
            if not override:
                raise wolfcord.ClientException(f"Cog named {cog_name!r} already loaded")
            self.remove_cog(cog_name)

        cog = cog._inject(self)
        self.__cogs[cog_name] = cog
        # TODO: This blind call to wolfcord.Client is dumb.
        ...
        # this being a "blind call" to wolfcord.Client
        ...
        try:
            if asyncio.iscoroutinefunction(setup):
                try:
                    ...
                    https://docs.wolfcord.dev/en/stable/faq.html#how-do-i-make-my-setup-function-a-coroutine-and-load-it
                    """
                    )
            else:
                setup(self, **extras)
    ...
    async def get_prefix(self, message: Message) -> Union[List[str], str]:
        ...
        Parameters
        ----------
        message: :class:`wolfcord.Message`
        ...
        if callable(prefix):
            ret = await wolfcord.utils.maybe_coroutine(prefix, self, message)  # type: ignore
        ...
        if isinstance(prefix, str):
            ...
            try:
                ...
                if message.content.startswith(tuple(prefix)):
                    invoked_prefix = wolfcord.utils.find(view.skip_string, prefix)
                else:
                    return ctx
...
class Bot(BotBase, wolfcord.Client):
    """Represents a discord bot.

    This class is a subclass of :class:`wolfcord.Client` and as a result
    anything that you can do with a :class:`wolfcord.Client` you can do with
    this bot.
    ...
    command_prefix
        ...
        as its first parameter and :class:`wolfcord.Message` as its second
    ...
    def __init__(
        ...
        *,
        ...
        intents: wolfcord.Intents = wolfcord.Intents.default(),
...
class AutoShardedBot(BotBase, wolfcord.AutoShardedClient):
    """This is similar to :class:`.Bot` except that it is inherited from
    :class:`wolfcord.AutoShardedClient` instead.
    """
    ...
    def __init__(
        ...
        *,
        ...
        wolfcord.AutoShardedClient.__init__(
            self,
            max_messages=max_messages,
            connector=connector,
            proxy=proxy,
